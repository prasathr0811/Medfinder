...
// Model Imports (for auto-seeding check)
...
// Base Route
app.get('/', (req, res) => {
  res.json({ message: 'MedFinder API is running' });
});

// Database Seeding Logic (run if empty)
const autoSeedDatabase = async () => {
  try {
    ...
    console.log('Database is empty. Auto-seeding initial MedFinder data...');

    // Seed 1 Admin, 3 Owners, 5 Customers
    ...
    // Seed 50+ Medicines distributed across the pharmacies
    ...
    for (let i = 0; i < 105; i++) {
      ...
      reservationDate.setDate(reservationDate.getDate() - (i % 15));
    ...
    console.log('Database successfully auto-seeded with 10 pharmacies, 50+ medicines, and 100+ reservations.');
  } catch (err) {
...
// Connect to MongoDB
mongoose.connect(process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/medfinder')
  .then(async () => {
